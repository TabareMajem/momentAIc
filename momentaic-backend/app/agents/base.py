...
class BaseAgent(ABC):
    ...
    async def structured_llm_call(
        ...
        If include_cot=True, forces the agent to output its reasoning explicitly,
        ...
        if response_model and hasattr(llm, "with_structured_output"):
            if include_cot:
                ...
                # Dynamically weave a Chain of Thought wrapper around the requested schema
                CotWrapper = create_model(
                    'CotWrapper',
                    _chain_of_thought=(Dict[str, Any], Field(
                        ..., 
                        description="Your step-by-step reasoning. Must include keys: 'research_analyzed', 'options_considered', and 'final_decision_rationale'."
                    )),
                    result=(response_model, Field(..., description="The final structured output satisfying the user's request."))
                )
                
                structured_llm = llm.with_structured_output(CotWrapper)
                response = await structured_llm.ainvoke(prompt)
                
                # Check for Langchain returning dicts vs objects depending on model parser
                if isinstance(response, dict):
                    return response_model.model_validate(response.get("result", {})), response.get("_chain_of_thought", {})
                else:
                    return response.result, getattr(response, "_chain_of_thought", {})
