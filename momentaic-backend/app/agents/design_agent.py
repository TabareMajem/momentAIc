...
# Singleton
    ...
    async def proactive_scan(self, startup_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Proactively scan for design trends and brand consistency issues.
        """
        actions = []
        ...
        results = await web_search(f"{industry} design trends and brand consistency issues 2025")
...
Identify the top 3 actionable insights. Be concise."""
                try:
                    ...
                    actions.append({"name": "design_audit_needed", "industry": industry})
                except Exception as e:
                    logger.error(f"DesignAgent proactive scan failed", error=str(e))
        
        return actions

    async def autonomous_action(self, action: Dict[str, Any], startup_context: Dict[str, Any]) -> str:
        """
        Generates design assets (social images, ad creatives, brand materials).
        """
        ...
        try:
            ...
            prompt = f"""You are the UI/UX design generation via AI image models agent for a {industry} startup.
...
Generate a concrete, actionable deliverable. No fluff. Be specific and executable."""
            ...
            logger.error("DesignAgent autonomous action failed", action=action_type, error=str(e))
