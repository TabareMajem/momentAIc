...
# Singleton instance  
    ...
    async def proactive_scan(self, startup_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Proactively scan for campaign opportunities and seasonal events.
        """
        actions = []
        ...
        results = await web_search(f"{industry} campaign opportunities and seasonal events 2025")
                ...
                try:
                    ...
                    if hasattr(self, 'publish_to_bus'):
                        await self.publish_to_bus(
                            ...
                            data={
                                "source": "ViralCampaignAgent",
                                "analysis": response.content[:1500],
                                "agent": "viral_campaign_agent",
                            }
                        )
                    actions.append({"name": "campaign_opportunity_found", "industry": industry})
                except Exception as e:
                    logger.error(f"ViralCampaignAgent proactive scan failed", error=str(e))
        
        return actions

    async def autonomous_action(self, action: Dict[str, Any], startup_context: Dict[str, Any]) -> str:
        """
        Auto-generates multi-format campaign assets (emails, social posts, landing copy).
        """
        ...
        try:
            ...
            prompt = f"""You are the Campaign asset generation and launch coordination agent for a {industry} startup.
...
Generate a concrete, actionable deliverable. No fluff. Be specific and executable."""
            ...
            logger.error("ViralCampaignAgent autonomous action failed", action=action_type, error=str(e))
