...
# Legacy singleton export
    ...
    async def proactive_scan(self, startup_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Proactively scan for marketing channel performance and new distribution opportunities.
        """
        actions = []
        ...
        from app.agents.base import web_search
        results = await web_search(f"{industry} marketing channel performance and new distribution opportunities 2025")
...
Identify the top 3 actionable insights. Be concise."""
                try:
                    ...
                    actions.append({"name": "channel_opportunity_found", "industry": industry})
                except Exception as e:
                    logger.error(f"MarketingAgent proactive scan failed", error=str(e))
        
        return actions

    async def autonomous_action(self, action: Dict[str, Any], startup_context: Dict[str, Any]) -> str:
        """
        Generates marketing hooks, distribution plans, and channel-specific content.
        """
        ...
        try:
            ...
            prompt = f"""You are the Marketing distribution and growth hacking agent for a {industry} startup.
...
Generate a concrete, actionable deliverable. No fluff. Be specific and executable."""
            ...
            logger.error("MarketingAgent autonomous action failed", action=action_type, error=str(e))
