...
# Singleton instance
    ...
    async def proactive_scan(self, startup_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Proactively scan for partnership opportunities and deal pipeline health.
        """
        actions = []
        ...
        results = await web_search(f"{industry} partnership opportunities and deal pipeline health 2025")
                ...
                try:
                    ...
                    if hasattr(self, 'publish_to_bus'):
                        await self.publish_to_bus(
                            ...
                            data={
                                "source": "DealmakeAgent",
                                "analysis": response.content[:1500],
                                "agent": "dealmaker",
                            }
                        )
                    actions.append({"name": "deal_opportunity_found", "industry": industry})
                except Exception as e:
                    logger.error(f"DealmakeAgent proactive scan failed", error=str(e))
        
        return actions

    async def autonomous_action(self, action: Dict[str, Any], startup_context: Dict[str, Any]) -> str:
        """
        Drafts partnership proposals and manages deal pipeline progression.
        """
        ...
        try:
            ...
            prompt = f"""You are the Partnership negotiation and deal pipeline management agent for a {industry} startup.
...
Generate a concrete, actionable deliverable. No fluff. Be specific and executable."""
            ...
            logger.error("DealmakeAgent autonomous action failed", action=action_type, error=str(e))
