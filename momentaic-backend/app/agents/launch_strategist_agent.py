...
# Singleton instance
    ...
    async def proactive_scan(self, startup_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Proactively scan for optimal launch windows and platform trends.
        """
        actions = []
        ...
        results = await web_search(f"{industry} optimal launch windows and platform trends 2025")
                ...
                try:
                    ...
                    if hasattr(self, 'publish_to_bus'):
                        await self.publish_to_bus(
                            ...
                            data={
                                "source": "LaunchStrategistAgent",
                                "analysis": response.content[:1500],
                                "agent": "launch_strategist_agent",
                            }
                        )
                    actions.append({"name": "launch_window_detected", "industry": industry})
                except Exception as e:
                    logger.error(f"LaunchStrategistAgent proactive scan failed", error=str(e))
        
        return actions

    async def autonomous_action(self, action: Dict[str, Any], startup_context: Dict[str, Any]) -> str:
        """
        Generates comprehensive launch strategies with platform recommendations and timelines.
        """
        ...
        try:
            ...
            prompt = f"""You are the Product launch strategy and platform selection agent for a {industry} startup.
...
Generate a concrete, actionable deliverable. No fluff. Be specific and executable."""
            ...
            logger.error("LaunchStrategistAgent autonomous action failed", action=action_type, error=str(e))
