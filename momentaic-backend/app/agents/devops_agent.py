...
# Singleton
    ...
    async def proactive_scan(self, startup_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Proactively scan for infrastructure health, deployment issues, and security vulnerabilities.
        """
        actions = []
        ...
        results = await web_search(f"{industry} infrastructure health, deployment issues, and security vulnerabilities 2025")
...
Identify the top 3 actionable insights. Be concise."""
                try:
                    ...
                    actions.append({"name": "infra_issue_detected", "industry": industry})
                except Exception as e:
                    logger.error(f"DevOpsAgent proactive scan failed", error=str(e))
        
        return actions

    async def autonomous_action(self, action: Dict[str, Any], startup_context: Dict[str, Any]) -> str:
        """
        Generates infrastructure audit reports, deployment checklists, and security assessments.
        """
        ...
        try:
            ...
            prompt = f"""You are the Infrastructure monitoring and deployment automation agent for a {industry} startup.
...
Generate a concrete, actionable deliverable. No fluff. Be specific and executable."""
            ...
            logger.error("DevOpsAgent autonomous action failed", action=action_type, error=str(e))
