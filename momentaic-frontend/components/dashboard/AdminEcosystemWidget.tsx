...
import { Shield, Zap, Sparkles, Box, CircuitBoard, Globe, Code, Brain, Cpu, RefreshCw, Copy, Check, Crosshair, Network, Users, TrendingUp } from 'lucide-react';
...
export function AdminEcosystemWidget() {
    ...
    const [assemblingArmy, setAssemblingArmy] = useState(false);
    const [launchingGtm, setLaunchingGtm] = useState(false);
    ...
    const handleAssembleArmy = async () => {
        setAssemblingArmy(true);
        try {
            const token = localStorage.getItem('token');
            // Hardcode startup ID for demo purposes, or fetch from context. "1" is a safe fallback in this mock environment.
            const startupId = "1";
            const res = await fetch('/api/v1/momentaic/campaigns/swarm/influencer-army', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                ...
                body: JSON.stringify({
                    ...
                    niche: "AI SaaS and Tech Founders",
                    region: "Global"
                })
            });
            if (res.ok) {
                alert("The KOL Headhunter and Ambassador Agents have been deployed. Watch the global feed.");
            } else {
                alert("Failed to assemble army.");
            }
        } catch (e) {
            console.error(e);
            alert("Swarm deployment failed.");
        } finally {
            setAssemblingArmy(false);
        }
    };

    const handleGtmSwarm = async () => {
        setLaunchingGtm(true);
        try {
            const token = localStorage.getItem('token');
            const res = await fetch('/api/v1/momentaic/campaigns/swarm/gtm-hunter', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                ...
                body: JSON.stringify({
                    target_company: "Sequoia Capital or Vercel",
                    campaign_type: "investment_reach",
                    custom_instructions: "Find key decision makers and generate highly personalized pitches based on their recent investments or product launches."
                })
            });
            if (res.ok) {
                alert("GTM Investor Reach Swarm initialized! Lead Researcher agents are hunting targets.");
            } else {
                alert("Failed to launch GTM Swarm.");
            }
        } catch (e) {
            console.error(e);
            alert("GTM Swarm deployment failed.");
        } finally {
            setLaunchingGtm(false);
        }
    };
        ...
        <div className="bg-[#050510] border border-cyan-500/10 rounded-3xl p-8 mb-12 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-10">
                    <div>
                        <h3 className="text-3xl font-black flex items-center gap-3 text-white tracking-tighter mb-1">
                            <div className="p-2 bg-[#111111] from-cyan-900/50 rounded-lg border border-white/10">
                                <Shield className="w-8 h-8 text-white" />
                            </div>
                            <span className="bg-clip-text text-transparent bg-[#111111] from-white ">
                                EMPIRE CONSOLE
                            </span>
                        </h3>
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-3">
                        <Button
                            ...
                            className="border-[#222222] text-purple-400 hover:bg-purple-500/10 hover:border-purple-500/60 hover:text-white transition-all h-12 px-6"
                        >
                        ...
                        <Button
                            variant="outline"
                            size="lg"
                            onClick={handleGtmSwarm}
                            disabled={launchingGtm}
                            className="border-emerald-500/30 text-emerald-500 hover:bg-emerald-500/10 hover:border-emerald-500/60 transition-all h-12 px-5"
                        >
                            {launchingGtm ? <RefreshCw className="w-5 h-5 animate-spin mr-2" /> : <TrendingUp className="w-5 h-5 mr-1" />}
                            INVESTOR OUTREACH
                        </Button>
                        <Button
                            variant="outline"
                            size="lg"
                            onClick={handleAssembleArmy}
                            disabled={assemblingArmy}
                            className="border-rose-500/30 text-rose-500 hover:bg-rose-500/10 hover:border-rose-500/60 transition-all h-12 px-5"
                        >
                            {assemblingArmy ? <RefreshCw className="w-5 h-5 animate-spin mr-2" /> : <Users className="w-5 h-5 mr-1" />}
                            VIRAL MARKETING
                        </Button>
                        <Button
                            variant="outline"
                            ...
                            className="border-yellow-500/30 text-yellow-500 hover:bg-yellow-500/10 hover:border-yellow-500/60 transition-all h-12 px-5"
                        >
                            {strategyLoading ? <RefreshCw className="w-5 h-5 animate-spin mr-2" /> : <Sparkles className="w-5 h-5 mr-1" />}
                            SURPRISE
                        </Button>
                    </div>
                </div>
                ...
                {strategyContent && (
                    ...
                    >
                        <div className="bg-[#111111] from-yellow-500/5 to-transparent border-l-4 border-yellow-500 rounded-r-xl p-6 relative">
 ...
 ${selectedProduct?.name === p.name
                                    ? 'bg-[#111111] from-cyan-900/40 border-cyan-400 scale-105 z-10'
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-8 relative z-10">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mt-4 sm:mt-0">
                                ...
                                <Button
                                    size="lg"
                                    className="bg-red-600 hover:bg-red-700 text-white border-0 hover: font-bold tracking-wider"
                                ...
                                {[
                                    ...
                                    <div key={item.key} className="bg-black/40 p-6 rounded-xl border border-white/5 hover:border-white/10 transition-colors group">
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className="absolute bottom-0 left-0 w-full h-8 bg-[#111111] from-black/0 to-transparent pointer-events-none" />
                                        </div>
                                    </div>
                                ))}
                            </motion.div>
