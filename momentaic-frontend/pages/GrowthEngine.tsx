...
function CRMBoard({ startupId, autopilot }: { startupId: string, autopilot: boolean }) {
    ...
    const generateOutreach = async () => {
        ...
        setGenerating(true);
        try {
            // [REALITY UPGRADE] Call Real Sales Agent
            const response = await api.generateOutreach(startupId, selectedLead.id);
            setAiAnalysis(response.body || response.subject || "No draft generated.");
        } catch (e) {
