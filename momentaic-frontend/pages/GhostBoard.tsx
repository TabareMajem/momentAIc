...
import { Sparkles, CheckCircle2, AlertTriangle, ArrowRight, Loader2, Zap, Rocket, Terminal, PlayCircle, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export default function GhostBoard() {
    ...
    const [moveRatings, setMoveRatings] = useState<Record<string, 'up' | 'down'>>({});

    // Hard Paywall Check for Starter Tier
    if (user?.subscription_tier === 'starter') {
        return (
            <div className="min-h-[80vh] flex flex-col items-center justify-center p-6 text-center animate-fade-in relative overflow-hidden">
                <div className="absolute inset-0 bg-purple-900/10 blur-[100px] rounded-full"></div>
                
                <div className="bg-[#060c18] border border-red-500/30 p-8 rounded-2xl max-w-lg w-full relative z-10 shadow-[0_0_50px_rgba(255,0,0,0.1)] backdrop-blur-xl">
                    <div className="w-16 h-16 bg-red-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6 text-red-500 shadow-inner">
                        <AlertTriangle className="w-8 h-8 animate-pulse" />
                    </div>
                    
                    <h2 className="text-3xl font-black text-white mb-2 uppercase tracking-tighter">Access Restricted</h2>
                    <p className="text-gray-400 text-sm mb-8 leading-relaxed">
                        Your onboarding scan is complete, and the baseline matrix is established. 
                        However, <strong className="text-purple-400">Autonomous Core Features</strong> and <strong className="text-deerflow-cyan">DeerFlow Engine</strong> operations require active compute credits.
                    </p>
                    
                    <div className="space-y-4">
                        <Button 
                            onClick={() => upgradeTier('growth')} 
                            className="w-full h-14 bg-gradient-to-r from-[#00e5ff] to-blue-600 text-white font-bold tracking-widest uppercase hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(0,229,255,0.3)]"
                        >
                            <Zap className="w-5 h-5 mr-2" /> Upgrade to Lite ($19/mo)
                        </Button>
                        <p className="text-[10px] text-gray-500 font-mono uppercase tracking-widest">Unlocks DeerFlow V3 & Core Engines</p>
                    </div>
                </div>
            </div>
        );
    ...
    // Trigger Wow Tour
    const { startTour } = useWowTour();
    useEffect(() => {
        // Small delay to ensure layout is painted
        const timer = setTimeout(() => {
            startTour('ghost_board_tour');
        }, 800);
    ...
    const handleAuthorize = async (move: Move) => {
        ...
        try {
            const resp = await (api as any).client.post('/api/v1/proactive/execute-action', {
                action_id: move.id,
    ...
    const handleRateMove = async (moveId: string, rating: 'up' | 'down') => {
        setMoveRatings(prev => ({ ...prev, [moveId]: rating }));
        // Mock API call to store feedback
        // await api.submitMoveFeedback(moveId, rating);
    };
    ...
    return (
        <div className="max-w-4xl mx-auto py-8">
            <div className="flex items-center justify-between mb-8">
                ...
                <button
                    onClick={() => startTour('ghost_board_tour')}
                    className="text-purple-400 hover:text-white transition-colors bg-purple-500/10 hover:bg-purple-500/20 rounded-full p-2 group"
                    title="Replay Ghost Board Tour"
                >
            ...
            <div className="bg-[#0A0A0A] border border-[#222222] rounded-xl overflow-hidden font-mono text-sm shadow-2xl">

                {/* Header Tape */}
                <div className="bg-[#111111] border-b border-[#222222] px-6 py-3 flex justify-between items-center text-xs text-gray-400">
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            {brief.today_moves.map((move, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex flex-col border border-[#222222] rounded-lg overflow-hidden bg-[#111111]/50 hover:border-[#222222] transition-colors mb-4">
                                        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center p-4">
                                            ...
                                            <div className="w-full sm:w-auto flex-shrink-0 shrink-0">
                                                {state === 'done' ? (
                                                    ...
                                                    <Button
                                                        onClick={() => handleAuthorize(move)}
                                                        className="w-full sm:w-[140px] bg-purple-600 hover:bg-purple-500 text-white font-mono text-xs scale-100 active:scale-95 transition-all h-9 uppercase group"
                                                    >
                                        ...
                                        {generatedAssets[move.id] && (
                                            <div className="p-4 border-t border-[#222222] bg-black/20 space-y-3">
                                                <GeneratedImage
                                                ...
                                                <Button
                                                    ...
                                                    className="w-full sm:w-auto h-10 px-6 bg-[#111111] from-pink-600 hover:from-pink-500 hover: text-white text-xs font-bold font-mono rounded-lg transition-all flex items-center gap-2 uppercase tracking-wider"
                                                >
                                        ...
                                        {/* PMF Feedback Footer for the Move */}
                                        <div className="flex items-center justify-between px-4 py-2 bg-[#0A0A0A] border-t border-[#222222]/50">
                                            <span className="text-[10px] uppercase font-mono tracking-widest text-gray-500">
                                                Is this move valuable?
                                            </span>
                                            <div className="flex gap-1">
                                                <button 
                                                    onClick={() => handleRateMove(move.id, 'up')}
                                                    className={`p-1.5 rounded-md transition-colors ${moveRatings[move.id] === 'up' ? 'bg-emerald-500/20 text-emerald-400' : 'text-gray-500 hover:text-emerald-400 hover:bg-emerald-500/10'}`}
                                                >
                                                    <ThumbsUp className="w-3.5 h-3.5" />
                                                </button>
                                                <button 
                                                    onClick={() => handleRateMove(move.id, 'down')}
                                                    className={`p-1.5 rounded-md transition-colors ${moveRatings[move.id] === 'down' ? 'bg-red-500/20 text-red-400' : 'text-gray-500 hover:text-red-400 hover:bg-red-500/10'}`}
                                                >
