...
export default function AgentsMarket() {
    ...
    const AgentCard: React.FC<{ agent: AgentInfo }> = ({ agent }) => {
        ...
        return (
            <Card className={`h-full flex flex-col group relative overflow-hidden ${isLocked ? 'opacity-75 bg-gray-50/5' : 'hover: border-white/10'}`}>
                {/* Hover Glow */}
                {!isLocked && <div className="absolute inset-0 bg-[#111111] from-[#00f0ff]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>}

                <CardHeader>
                    <div className="flex justify-between items-start">
                        <div className={`p-3 rounded-xl border ${isLocked ? 'bg-[#111111] border-white/5' : 'bg-[#00f0ff]/10 border-[#00f0ff]/20'}`}>
    ...
    return (
        <div className="space-y-8 min-h-[80vh]">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-[#0a0a0a] border border-white/10 p-8 md:p-12 text-center">
                ...
                <h1 className="relative z-10 text-3xl md:text-4xl font-black text-white mb-2 uppercase tracking-tighter flex items-center justify-center gap-3">
                    Agent Command Center
                    <button
                        onClick={() => startTour('agent_market_tour')}
                        className="text-[#00f0ff] hover:text-white transition-colors bg-[#00f0ff]/10 hover:bg-[#00f0ff]/20 rounded-full p-2 group"
                        title="Replay Agent Market Tour"
                    >
                ...
                <div className="relative z-10 max-w-2xl mx-auto">
                    <div className="relative group">
                        <div className="absolute -inset-0.5 bg-[#111111] from-[#00f0ff] to-[#a855f7] rounded-xl opacity-30 group-hover:opacity-70 blur transition duration-500"></div>
