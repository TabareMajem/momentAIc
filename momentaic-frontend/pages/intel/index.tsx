...
const IntelCard = ({ article }: { article: any }) => (
 <Link to={`/intel/${article.slug}`} className="group block border border-white/10 bg-[#0A0A0A] hover:border-purple-500/50 transition-all clip-corner-4 relative overflow-hidden flex flex-col h-full">
 {article.featuredImage && (
 <div className="w-full h-48 overflow-hidden relative">
 <div className="absolute inset-0 bg-[#111111] from-[#0a0a0f] to-transparent z-10" />
 <img
 ...
 <div className="inline-flex items-center gap-2 px-2 py-1 bg-[#0A0A0A]/90 border border-white/10 rounded text-[10px] font-mono text-purple-400 mb-4 backdrop-blur-sm self-start">
...
const LockedCard = ({ title, category }: { title: string, category: string }) => (
 <div className="group block border border-white/5 bg-[#0A0A0A] p-6 clip-corner-4 relative overflow-hidden opacity-50 cursor-not-allowed h-full min-h-[300px] flex flex-col">
...
export default function IntelHub() {
 ...
 <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tighter bg-clip-text text-transparent bg-[#111111] from-white ">
