    ...
    return (
        <div className="relative w-full max-w-sm mx-auto mt-8 touch-none">
            ...
            >
                {/* Progress Fill */}
                <div
                    className="absolute left-0 top-0 bottom-0 bg-gradient-to-r from-[#111111] to-purple-600 transition-all duration-75 ease-out"
                ...
                return (
                    <div key={i} className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <div className={cn(
                            ...
                            isActive ? "bg-purple-500/20 border-purple-500 text-white " :
...
export default function GeniusOnboarding() {
    ...
    // Stream native backend metrics to UI
    const { activities } = useAgentStream(startupId || undefined);

    useEffect(() => {
        if (step !== 2) return;
        const geniusActivities = [...activities].reverse().filter(a => a.agent === 'genius');
        
        let visualSteps: string[] = [];
        if (geniusActivities.some(a => a.action === 'initializing')) {
            visualSteps.push(`[${AGENT_STEPS[0].name}] ${AGENT_STEPS[0].log}`);
            visualSteps.push(`[${AGENT_STEPS[1].name}] ${AGENT_STEPS[1].log}`);
        }
        if (geniusActivities.some(a => a.action === 'scanning')) {
            visualSteps.push(`[${AGENT_STEPS[2].name}] ${AGENT_STEPS[2].log}`);
            visualSteps.push(`[${AGENT_STEPS[3].name}] ${AGENT_STEPS[3].log}`);
        }
        if (geniusActivities.some(a => a.action === 'session_created')) {
            visualSteps.push(`[${AGENT_STEPS[4].name}] ${AGENT_STEPS[4].log}`);
            visualSteps.push(`[${AGENT_STEPS[5].name}] ${AGENT_STEPS[5].log}`);
        }
        
        if (visualSteps.length > analysisLines.length) {
             setAnalysisLines(visualSteps);
        }
    }, [activities, step]);
    ...
    const autoExecuteAnalysis = async (urlToAnalyze: string, currentStartupId: string | null) => {
        ...
        try {
            // Actual API Call - WebSocket connection will drive the visual progression
            ...
            // Ensure 100% completion state is rendered upon strict API fulfillment
            setAnalysisLines(AGENT_STEPS.map(a => `[${a.name}] ${a.log}`));

            // Artificial 1-second pause to let the user see 100% completion graphic before shifting layout
            setTimeout(() => {
                setPlan(parsedPlan);
                setStep(3);
                setLoading(false);
    ...
    const stepVariants = {
        ...
        visible: { opacity: 1, x: 0, scale: 1, transition: { type: 'spring' as any, stiffness: 300, damping: 30 } },
    ...
    const cardVariants = {
        ...
        visible: (i: number) => ({
            opacity: 1,
            y: 0,
            transition: { delay: i * 0.15, type: 'spring' as any, stiffness: 300, damping: 25 }
        })
    };
    ...
    return (
        <div className="min-h-screen bg-[#020202] text-white flex flex-col p-4 sm:p-6 overflow-hidden relative selection:bg-purple-500/30">
            ...
            <div className="w-full max-w-4xl mx-auto relative z-10 flex-1 flex flex-col justify-start sm:justify-center pt-8 sm:pt-0">
                ...
                >
                    <h1 className="text-3xl sm:text-5xl font-black mb-2 sm:mb-4 tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-300 to-gray-500">
                        ...
                        {step === 1 && (
                            ...
                            >
                                <div className="bg-[#08080c]/80 backdrop-blur-2xl border border-white/5 p-6 sm:p-12 rounded-[2rem] text-center max-w-2xl mx-auto shadow-2xl overflow-hidden relative">
                                    {/* Inner subtle glow */}
                                    <div className="absolute inset-0 bg-[#111111] from-purple-500/5 to-transparent pointer-events-none" />
                                    ...
                                    <motion.button
                                        ...
                                        className={cn(
                                            ...
                                            inputValue.trim()
                                                ? "bg-gradient-to-r from-[#111111] to-purple-600 text-white hover:opacity-90"
                            ...
                            >
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8 w-full max-w-2xl">
                                    {AGENT_STEPS.map((agent, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {isCurrent && <div className="absolute inset-0 bg-[#111111] from-purple-500/10 to-transparent animate-pulse" />}
                                ...
                                <div className="relative w-32 h-32 sm:w-40 sm:h-40 flex items-center justify-center mb-6">
                                    ...
                                    <motion.div
                                        ...
                                        className="absolute inset-6 bg-gradient-to-r from-[#111111] to-purple-600 rounded-full blur-xl"
                                    />
                                    <div className="relative z-10 w-14 h-14 sm:w-16 sm:h-16 bg-black rounded-full border border-white/20 flex items-center justify-center ">
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                                    ...
                                    <motion.div
                                        ...
                                        className="lg:col-span-3 bg-[#0A0A0A]/90 backdrop-blur-xl border border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group hover:border-purple-500/20 transition-colors"
                                    >
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#111111] to-purple-500 flex items-center justify-center">
                                    ...
                                    <motion.div
                                        ...
                                        className="bg-black/50 backdrop-blur-md border border-white/5 p-6 rounded-3xl hover:border-[#222222] hover: transition-all duration-300"
                                    >
                                    ...
                                    <motion.div
                                        ...
                                        className="bg-black/50 backdrop-blur-md border border-white/5 p-6 rounded-3xl hover:border-emerald-500/30 hover: transition-all duration-300"
                                    >
