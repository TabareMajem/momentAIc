...
import {
    ...
    ExternalLink, Edit, Trash2, Plus, Loader
} from 'lucide-react';
...
function CampaignCard({ template, onStart, loading }: { template: CampaignTemplate; onStart: () => void; loading?: boolean }) {
    ...
    return (
        <Card className="p-5 bg-black/50 border-white/10 hover:border-[#00f0ff]/30 transition-all">
            ...
            <div className="flex gap-2">
                <Button onClick={onStart} variant="cyber" size="sm" className="flex-1" disabled={loading}>
                    {loading ? <Loader className="w-3 h-3 mr-2 animate-spin" /> : <Play className="w-3 h-3 mr-2" />}
                    {loading ? 'Designing...' : 'Start Campaign'}
                </Button>
...
export default function CampaignCenter() {
    ...
    const handleStartCampaign = async (template: CampaignTemplate) => {
        setGeneratingId(template.id);
        toast({
            type: 'info',
            title: 'Generating Campaign Plan...',
            message: 'The Marketing Agent is crafting your custom strategy.'
        });

        try {
            const token = localStorage.getItem('access_token');
            const response = await fetch('/api/v1/growth/campaigns/generate', {
                method: 'POST',
                headers: {
            ...
            toast({
                ...
                message: `"${plan.name}" has been created with ${plan.tasks.length} custom tasks.`
            });
            // In a real app, we'd add this to the active campaigns list state
