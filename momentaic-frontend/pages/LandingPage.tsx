...
const SectionHeader = ({ title, subtitle, id }: { title: string, subtitle: string, id: string }) => (
 ...
 <div className="absolute top-0 left-0 w-full h-px bg-[#111111] from-transparent to-transparent" />
...
const ModuleCard = ({ icon: Icon, title, desc, delay, visible = true }: { icon: any, title: string, desc: string, delay: number, visible?: boolean }) => (
 ...
 <div className="relative h-full bg-[#0A0A0A] p-6 clip-corner-4 overflow-hidden">
...
const AgentBadge = ({ agent, delay = 0, visible = true }: { agent: any, delay?: number, visible?: boolean }) => (
 <div
 className={`min-w-[300px] bg-[#0A0A0A] border border-white/10 clip-id-card relative group hover:border-purple-500/40 transition-all duration-700 transform ${visible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}
 style={{ transitionDelay: `${delay}ms` }}
 >
 {/* Holographic Header */}
 <div className="h-24 bg-[#111111] to-transparent p-4 flex justify-between items-start relative overflow-hidden">
...
export default function LandingPage() {
 ...
 <section id="roster" className="py-32 border-y border-white/5 bg-[#0A0A0A] relative overflow-hidden">
 ...
 <div className="absolute inset-0 bg-[#111111] via-transparent " />
 <div className="max-w-7xl mx-auto relative z-10">
 ...
 {/* ─── AMBASSADOR CTA ─── */}
 <section className="py-20 px-6 bg-[#111111] from-[#0a0014] via-[#050508] to-[#000a14] border-y border-white/5">
