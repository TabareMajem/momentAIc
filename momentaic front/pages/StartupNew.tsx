  ...
  const handleSend = async (e: React.FormEvent) => {
      ...
      switch (step) {
          case 0: // Name -> Description
              ...
              setStep(1);
              break;
          case 1: // Description -> Industry
              addAiMessage("Analyzing market vectors... Valid sector detected. Which industry category best fits this operation (e.g., FinTech, SaaS, BioTech)?", 'industry');
              setStep(2);
              break;
          case 2: // Industry -> Stage
              addAiMessage("Understood. What is the current operational status? (Idea, MVP, Growth, Scale)", 'stage');
              setStep(3);
              break;
          case 3: // Stage -> Finalize
              nextData.stage = userText.toLowerCase().includes('mvp') ? 'mvp' : 
                               userText.toLowerCase().includes('growth') ? 'growth' : 'idea';
              setFormData(nextData); // Update local state strictly
              
              setIsTyping(true);
              setTimeout(() => finalizeCreation(nextData), 1500);
              break;
      }
  };
